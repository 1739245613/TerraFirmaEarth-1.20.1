...
import com.newterraearth.tfe.client.animation.NTEBactrianCamelAnimation;
import com.newterraearth.tfe.common.entity.NTEBactrianCamel;


public class NTEBactrianCamelModel extends HierarchicalAnimatedModel<NTEBactrianCamel>
{
    ...
    public NTEBactrianCamelModel(ModelPart root) {
        super(root);
    ...
    @Override
    public void setupAnim(NTEBactrianCamel animal, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch)
    {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.applyHeadRotation(netHeadYaw, headPitch);
        wool_body.visible = animal.hasProduct();
        wool_shey.visible = animal.hasProduct();
        // The Camel entity exposes these synchronized animation states on both
        // the client and server. Keeping all six upstream clips here is
        // important for riding: the dash state drives the same pose transition
        // that the vanilla rider positioning code uses during a jump.
        this.animateWalk(NTEBactrianCamelAnimation.CAMEL_WALK, limbSwing, limbSwingAmount, 2.0F, 2.5F);
        this.animate(animal.sitAnimationState, NTEBactrianCamelAnimation.CAMEL_SIT, ageInTicks, 1.0F);
        this.animate(animal.sitPoseAnimationState, NTEBactrianCamelAnimation.CAMEL_SIT_POSE, ageInTicks, 1.0F);
        this.animate(animal.sitUpAnimationState, NTEBactrianCamelAnimation.CAMEL_STANDUP, ageInTicks, 1.0F);
        this.animate(animal.idleAnimationState, NTEBactrianCamelAnimation.CAMEL_IDLE, ageInTicks, 1.0F);
        this.animate(animal.dashAnimationState, NTEBactrianCamelAnimation.CAMEL_DASH, ageInTicks, 1.0F);
    }
    ...
    {
        // TFC's dedicated renderer leaves baby scaling to this model. Using
        // SimpleMobRenderer here would apply another 0.7 factor and make
        // young Bactrian camels too small compared with their hitbox.
        if (this.young)
        {
            poseStack.pushPose();
